...
// Placeholder verses generator
export function getVerses(book: string, chapter: number): string[] {
  const count = Math.floor(Math.random() * 15) + 10;
  return Array.from({ length: count }, (_, i) =>
    `This is verse ${i + 1} of ${book} chapter ${chapter}. The actual KJV text will be inserted here. "For God so loved the world, that he gave his only begotten Son, that whosoever believeth in him should not perish, but have everlasting life."`
  );
}
